...
for c in SAMPLE:
    ...
    rows.append(dict(id=c["id"], retrieval_s=round(t1 - t0, 3),
...
def s(k):
    ...
    return dict(mean=round(statistics.mean(v), 3), median=round(statistics.median(v), 3),
...
out = {"n": len(rows), "retrieval_s": s("retrieval_s"), "generation_s": s("generation_s"),
       "end_to_end_s": {k: round(v, 3) for k, v in
                        dict(mean=statistics.mean(r["retrieval_s"] + r["generation_s"] for r in rows),
