...
out = dict(
    ...
    current_design=dict(calls=len(kept), input_tokens=batched_tok, cost_usd=round(cost(batched_tok, len(kept)), 6)),
    per_chunk_same_files=dict(calls=chunk_calls, input_tokens=chunk_tok, cost_usd=round(cost(chunk_tok, chunk_calls), 6)),
    per_chunk_no_filter=dict(calls=nofilter_calls, input_tokens=nofilter_tok, cost_usd=round(cost(nofilter_tok, nofilter_calls), 6)),
)
